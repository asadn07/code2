...
A valid plaintext (=the text to be encoded) may only contain the letters A-Z (case insensitive, at least one letter) and spaces and must not exceed 50 characters (including spaces).
...
The decryption process is the reverse of the encryption with the following rules:

1. if both letters are in the same line of the Playfair square, they will be replaced with their *left* neighbor letter in the square. If there is a letter on the left edge of the square, it will be replaced with the letter on the right edge of the same line of the square.
...
Example with plaintext 'Programming is fun' with key from above (HALOVET)
```
